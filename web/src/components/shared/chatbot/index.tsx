...
export function ChatBotContainer() {
  const [selectedLanguage, setSelectedLanguage] = useState<"am" | "en">("en");
  const sendToChatbot = useMutation({
  ...
  const handleSendMessage = () => {
    ...
    sendToChatbot.mutateAsync(inputValue);
    setInputValue("");
  };
  ...
  return (
    <>
      ...
      <Button
        ...
        size="icon"
        className="fixed bottom-20 md:bottom-6 right-6 h-14 w-14 rounded-full shadow-lg hover:scale-110 transition-transform z-50"
      >
        {isOpen ? (
          <X className="h-6 w-6" />
        ) : (
      ...
      {/* Chat Window */}
      {isOpen && (
        <Card className="overflow-hidden fixed pt-0 bottom-24 right-6 w-[90vw] md:w-[380px] h-[70dvh] md:h-[500px] shadow-2xl z-50 flex flex-col border-border">
          ...
          <CardFooter className="border-t p-4">
            <div className="flex w-full gap-2">
              ...
              <Input
                placeholder="Ask about bookings, reports, or staff tasks..."
                value={inputValue}
                onChange={(e) => setInputValue(e.target.value)}
                onKeyPress={handleKeyPress}
                disabled={sendToChatbot.isPending}
